...
#### Using 32 node (128 gpus) bf16 precision
To execute the job with the default settings, run the following command from
your client:

```bash
cd $RECIPE_ROOT
export WORKLOAD_NAME=$USER-a4x-llama3-1-405b-32node
helm install $WORKLOAD_NAME . -f values.yaml \
--set-file workload_launcher=launcher.sh \
--set-file workload_config=llama3-1-405b-bf16-gbs64-gpu128.py \
...
```
kubectl get pods | grep $USER-a4x-llama3-1-405b-32node
```

Replace the following:

- JOB_NAME_PREFIX - your job name prefix. For example $USER-a4x-llama3-1-405b-32node.
...
For example: `$USER-a4x-llama3-1-405b-32node-workload-0-0-s9zrv`.
...
```bash
helm uninstall $USER-a4x-llama3-1-405b-32node
```
